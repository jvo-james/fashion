(() => {
  ...
  const state = {
    ...
    density: UI.storageGet('the_set_grid_density') || 'standard',
  ...
  const getProducts = () => {
    ...
    products = [...products].sort((a, b) => {
      ...
      if (state.sort === 'recommended') return Number(b.feature) - Number(a.feature) || b.rating - a.rating;
  ...
  const renderShop = () => {
    ...
    document.title = `${state.search ? `Search: ${state.search}` : config.label} - The Set`;

    main.innerHTML = `
      <section class="plp-intro">
        <p class="eyebrow">The Set / Shop</p>
        <h1>${state.search ? `Search: “${UI.escapeHTML(state.search)}”` : UI.escapeHTML(config.label)}</h1>
        <p>${state.search ? 'Stuff that matched your search.' : collectionCopy(state.category)}</p>
      </section>
        ...
        <div class="product-toolbar" data-product-toolbar>
          ...
          <div class="product-toolbar__actions">
            ...
            <div class="density-control" aria-label="Grid density">
              ...
              <button type="button" data-density="feature" class="${state.density === 'feature' ? 'is-active' : ''}" aria-label="Large grid">${UI.ICONS.grid2}</button>
            </div>
          </div>
        </div>
  ...
  const collectionCopy = (category) => ({
    women: 'Dresses, shirts and trousers for normal days and nights out.',
    men: 'Good jackets, knitwear and shirts. Nothing too stiff.',
    bags: 'Good bags with room for your stuff and nice leather.',
    shoes: 'Shoes for work, weekends and late nights.',
    accessories: 'Little things that finish an outfit.',
    jewelry: 'Necklaces, rings and small shiny things.',
    lifestyle: 'Home bits we would keep around ourselves.',
    gifts: 'Gifts that do not feel like filler.'
  }[category]);
  ...
  const updateCatalog = () => {
    ...
    grid.innerHTML = visible.length
      ...
      : `<div class="empty-results"><h3>No pieces found</h3><p>Nothing matched. Try taking a filter off.</p><button class="button button--black" type="button" data-clear-filters>Clear all filters</button></div>`;
  ...
  const openFilters = (trigger) => {
    ...
    const content = `
      <form class="filter-form" data-filter-form>
        ...
        ${filterGroup('highlight', 'Labels', options.highlight, pending.highlight, products)}
        <div class="filter-form__footer">
          <button class="text-button" type="button" data-reset-pending>Reset</button>
          <button class="button button--black" type="submit">Apply</button>
        </div>
      </form>`;
  ...
  const openSort = (trigger) => {
    const content = `
      <form class="sort-form" data-sort-form>
        ...
        <button class="button button--black button--full" type="submit">Apply</button>
      </form>`;
    const layer = UI.showModal({ title: 'Sort', content, id: 'sort-modal', extraClass: 'sort-modal' }, trigger);
  ...
  const wireShop = () => {
    ...
    document.querySelectorAll('[data-density]').forEach((button) => button.addEventListener('click', () => {
      ...
      UI.storageSet('the_set_grid_density', state.density);
