(() => {
  ...
  if (!A) throw new Error('The Set catalog failed to load.');

  const STORAGE = {
    cart: 'the_set_cart_v1',
    wishlist: 'the_set_wishlist_v1',
    announcement: 'the_set_announcement_hidden_v1'
  };
  ...
  const ICONS = {
    ...
    bag: `<svg class="bag-icon" viewBox="0 0 28 28" aria-hidden="true"><path d="M7.5 10.5 4.8 24h18.4l-2.7-13.5Z"/><path d="M9.5 11V8.5a4.5 4.5 0 0 1 9 0V11"/><path d="m9.2 16.2 4.8-2.7 4.8 2.7-4.8 2.7-4.8-2.7Z"/></svg>`
  };
      ...
      <header class="site-header ${announcementHidden ? 'site-header--no-announcement' : ''}">
        ...
        <div class="header-bar">
          ...
          <div class="header-actions">
            ...
            <button type="button" class="header-text-action" data-open-contact>Help</button>
            <button type="button" class="icon-button header-user" aria-label="Account">${ICONS.user}</button>
            <button type="button" class="bag-button" data-open-cart aria-label="Open bag, ${cartQuantity()} items">
              ${ICONS.bag}<span class="bag-count" data-cart-count>${cartQuantity()}</span>
            </button>
          </div>
        </div>
  ...
  const megaContent = (category) => {
    ...
    return `
      <div class="mega-menu__column">
        ...
        <a class="mega-menu__hero-link" href="shop.html?category=${category}">See everything ${ICONS.arrow}</a>
      </div>
      <div class="mega-menu__column">
      ...
      <div class="mega-menu__column">
        <p class="mega-menu__eyebrow">More stuff</p>
        <a href="shop.html?category=${category}&highlight=New">New stuff</a>
        <a href="shop.html?category=${category}&highlight=Back%20soon">Runway stuff</a>
        <a href="shop.html?category=${category}&highlight=Online%20only">Online only</a>
        <a href="shop.html?category=${category}&highlight=Popular">Favorites</a>
      </div>
      <a class="mega-menu__feature" href="product.html?id=${imageProduct.id}">
        <img src="${imageProduct.image}" alt="${escapeHTML(imageProduct.name)}" />
        <span>THE NIGHT DROP</span>
  ...
  const renderPersistentOverlays = (categories) => {
    ...
    overlays.innerHTML = `
      <section class="overlay-layer overlay-layer--drawer overlay-layer--left" id="mobile-nav" aria-hidden="true" hidden>
        ...
        <div class="overlay-layer__panel mobile-nav-panel" role="dialog" aria-modal="true" aria-label="Navigation menu">
          ...
          <nav class="mobile-nav" aria-label="Mobile navigation">
            ...
            <a href="shop.html?category=women&highlight=New"><span>New Arrivals</span>${ICONS.arrow}</a>
            <a href="shop.html?category=gifts"><span>Gifts</span>${ICONS.arrow}</a>
          </nav>
          <div class="mobile-nav__service">
            <button type="button" data-open-contact>Help</button>
      ...
      <section class="overlay-layer overlay-layer--drawer overlay-layer--right cart-layer" id="cart-layer" aria-hidden="true" hidden>
        <button class="overlay-layer__backdrop" type="button" data-close-layer aria-label="Close bag"></button>
        <div class="overlay-layer__panel" role="dialog" aria-modal="true" aria-labelledby="cart-title">
          <header class="overlay-layer__header">
            <h2 id="cart-title">Bag <span data-cart-heading-count>(${cartQuantity()})</span></h2>
            <button class="icon-button" type="button" data-close-layer aria-label="Close bag">${ICONS.close}</button>
          </header>
    ...
    footer.innerHTML = `
      <section class="newsletter-section" aria-labelledby="newsletter-title">
        <div>
          ...
          <h2 id="newsletter-title">Come say hi</h2>
          <p>A few emails when something new goes up. That is it.</p>
        </div>
      ...
      <footer class="site-footer" id="site-footer-inner">
        <div class="footer-grid">
          ${footerColumn('Help', ['Contact us', 'Delivery & returns', 'Track your order', 'Care', 'Book a time'])}
          ${footerColumn('About', ['About us', 'How we buy things', 'Work with us', 'Notes'])}
          ${footerColumn('Legal', ['Terms & conditions', 'Privacy', 'Cookies', 'Accessibility', 'Corporate information'])}
          ${footerColumn('See', ['Find a shop', 'Gifts', 'For regulars', 'New drops', 'Old stuff'])}
        </div>
        <div class="footer-bottom">
          <button class="footer-region" type="button" data-open-region>${ICONS.location}<span>Worldwide / English</span></button>
          <p>© 2026 The Set.</p>
          <p class="footer-favorite">Good clothes. That is kind of it.</p>
        </div>
      </footer>
    `;
  ...
  const showNewsletterModal = (email = '', trigger) => {
    const content = `
      <form class="expanded-form" data-newsletter-form>
        <p class="form-intro">Get a note when something new goes up. We will not fill your inbox.</p>
        ...
        <label class="consent"><input type="checkbox" required /><span>I agree to receive news from The Set and understand the privacy notice.</span></label>
        <button class="button button--black button--full" type="submit">Sign me up</button>
      </form>`;
    const layer = showModal({ title: 'Email list', content, extraClass: 'newsletter-modal' }, trigger);
    $('[data-newsletter-form]', layer).addEventListener('submit', (event) => {
      event.preventDefault();
      closeLayer();
      toast('Nice one. We will email you when something new is up.');
    });
  };
  ...
  const showContactDrawer = (trigger) => showDrawer({
    ...
    content: `
      <div class="service-list">
        <section><p class="eyebrow">Phone</p><h3>Talk to us</h3><p>Mon to Sat, 09:00 to 20:00 GMT</p><a href="tel:+233302000000">+233 30 200 0000</a></section>
        <section><p class="eyebrow">Email</p><h3>Email us</h3><p>We usually reply pretty quickly.</p><a class="button button--outline" href="mailto:hello@theset.example">Send email</a></section>
        <section><p class="eyebrow">Meet us</p><h3>Styling help</h3><p>Need help picking something? Ask us.</p><button class="button button--outline" type="button" data-book-appointment>Book now</button></section>
        <section><p class="eyebrow">Live chat</p><h3>We're here</h3><button class="button button--black" type="button" data-live-chat>Start chat</button></section>
      </div>`
  }, trigger);
  ...
  const showRegionModal = (trigger) => {
    ...
    const layer = showModal({
      ...
      id: 'region-modal',
      content: `
        <p class="form-intro">Prices are in USD here. Pick a place to see the shipping note.</p>
  ...
  const renderCart = () => {
    ...
    if (!state.cart.length) {
      host.innerHTML = `
        <div class="empty-cart">
          <span class="empty-cart__icon">${ICONS.bag}</span>
          <h3>Your bag is empty</h3>
          <p>Nothing here yet. Go find something you like.</p>
          <a class="button button--black" href="shop.html?category=women">See the shop</a>
        </div>`;
    } else {
      host.innerHTML = `
        ...
        <div class="cart-summary">
          ...
          <p>Delivery is free over $100. Returns are simple. Taxes depend on where you live.</p>
          <button class="button button--black button--full" type="button" data-checkout>Checkout</button>
    ...
    $('.bag-button')?.setAttribute('aria-label', `Open bag, ${cartQuantity()} items`);
    ...
    $$('[data-cart-remove]').forEach((button) => button.addEventListener('click', () => {
      state.cart.splice(Number(button.dataset.cartRemove), 1); persist(); renderCart(); announce('Item removed from bag.');
    }));
    $('[data-checkout]')?.addEventListener('click', () => toast('Checkout is a demo for now.'));
  };
  ...
  const quickAdd = (productId, trigger) => {
    ...
    const content = `
      ...
      <form data-quick-add-form>
        ...
        <button class="button button--black button--full" type="submit">Add to bag</button>
      </form>`;
